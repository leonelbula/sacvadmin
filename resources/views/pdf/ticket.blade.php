        ...
        <div class="card">
            ...
            <table class="meta">
                ...
                <tr>
                    <td class="k">Fecha</td>
                    <td class="v">
                        {{ \Carbon\Carbon::parse($sale->date ?? $sale->created_at)->format('d/m/Y H:i') }}</td>
                </tr>
                <tr>
                    <td class="k">Cajero</td>
                    <td class="v">{{ Auth::user()->name }}</td>
                </tr>
                <tr>
                    <td class="k">Cliente</td>
                    <td class="v">{{ $sale->customer->full_name ?? 'Consumidor final' }}</td>
                </tr>
                <tr>
                    <td class="k">Metodo de pago</td>
                    <td class="v">{{ $sale->payment_form == 'counted' ? 'Contado' : 'Credito' }}</td>
                </tr>
            </table>
            ...
            @if (isset($payments) && $payments->count())
                ...
                <table class="totals">
                    ...
                    @if ($sale->payment_form)
                        <tr>
                            <td class="k">Efectivo recibido</td>
                            <td class="v">${{ number_format($sale->total, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="k">Cambio</td>
                            <td class="v">${{ number_format($total, 2) }}</td>
                        </tr>
                    @endif
                </table>
            ...
            <!-- QR / Barcode opcional -->
            @if (!empty($sale->qr_base64))
                <img class="qrcode" src="data:image/png;base64,{{ $sale->qr_base64 }}" alt="QR">
            @endif
            @if (!empty($sale->barcode_base64))
                <img class="barcode" src="data:image/png;base64,{{ $sale->barcode_base64 }}" alt="Barcode">
            @endif
